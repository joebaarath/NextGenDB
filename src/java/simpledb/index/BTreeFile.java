...
public class BTreeFile implements DbFile {
	...
	public BTreeLeafPage splitLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		// Shift rest of tuples to new page
		int i = 0;
		Iterator<Tuple> tupleIterator = page.reverseIterator();
		Tuple thisTuple = null;
		while(tupleIterator.hasNext() && i < page.getMaxTuples()/2) {
			i += 1;
			thisTuple = tupleIterator.next();
			page.deleteTuple(thisTuple);
			newPage.insertTuple(thisTuple);
		}

		// Add key to parent page
		BTreeEntry entry = new BTreeEntry(thisTuple.getField(keyField), page.getId(), newPage.getId());
		BTreeInternalPage newParent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), entry.getKey());
		newParent.insertEntry(entry);

		// Setting the parent since child must point back to parent
		newPage.setParentId(newParent.getId());

		// Setting Siblings
		if(page.getRightSiblingId() != null) {
			BTreeLeafPage newRightSibling = (BTreeLeafPage) getPage(tid, dirtypages, page.getRightSiblingId(), Permissions.READ_WRITE);
			newRightSibling.setLeftSiblingId(newPage.getId());
			newPage.setRightSiblingId(newRightSibling.getId());
		}
		newPage.setLeftSiblingId(page.getId());
		page.setRightSiblingId(newPage.getId());

		// Compare then return page
	...
	public BTreeInternalPage splitInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		// Shift rest of entries to the new Internal Page
		int i = 0;
		Iterator<BTreeEntry> bteIterator = page.reverseIterator();
		BTreeEntry entry = null;
		while(bteIterator.hasNext() && i < page.getMaxEntries()/2) {
			i += 1;
			entry = bteIterator.next();
		...
		// get the last key into Parent
		entry = bteIterator.next();
		page.deleteKeyAndRightChild(entry);
		BTreeInternalPage newParent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), entry.getKey());
		newParent.insertEntry(new BTreeEntry(entry.getKey(), page.getId(), newInternalPage.getId()));

		// Set parents
		newInternalPage.setParentId(newParent.getId());
		updateParentPointers(tid, dirtypages, newInternalPage);

		// Compare and then return page
		if(entry.getKey().compare(Op.GREATER_THAN, field)) {
			return page;
		}
		else {
