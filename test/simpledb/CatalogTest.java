...
public class CatalogTest extends SimpleDbTestBase {
    ...
    public void getTableId() {
        ...
        Assert.assertEquals(id2, tableId);
    }
    ...
    public void getTableName() {
        assertEquals(name, Database.getCatalog().getTableName(id2));
        assertEquals(nameThisTestRun, Database.getCatalog().getTableName(id1));
        ...
        // additional test
        String tableName = Database.getCatalog().getTableName(id1);
        Assert.assertEquals(nameThisTestRun, tableName);
    }
     ...
     */
    @Test
    public void tableIdIterator() {
        int count = 0;
        Iterator<Integer> it = Database.getCatalog().tableIdIterator();
        while (it.hasNext()) {
            count++;
            int id = it.next();
            Assert.assertTrue(id == id1 || id == id2);
        }
    ...
    public void clear() {
        ...
        try {
            ...
            Assert.fail("Should not find table with name " + name);
        } catch (NoSuchElementException e) {
            // Expected to get here
        }
    }
